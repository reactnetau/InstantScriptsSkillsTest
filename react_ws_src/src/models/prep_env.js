...
const prep_env = function (callback) {
  ...
  function set_settings() {
    ...
    app.settings.is_mobile = /Android/i.test(ua) || /iPad|iPhone|iPod/i.test(ua) || /webOS|BlackBerry|Windows Phone|Opera Mini|IEMobile|windows mobile/i.test(ua);

    if (app.settings.is_mobile) {
      app.settings.mobile_type = /Android/i.test(ua) ? 'Android' :
        (/iPad|iPhone|iPod/i.test(ua) ? 'iOS' : null);
    }
